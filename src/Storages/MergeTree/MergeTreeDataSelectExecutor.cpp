...
#include <Storages/MergeTree/MergeTreeLogPartSource.h>
...
{
    /// clickhouse original query plan as main data,
    /// we add log part as delta data.

    /// query plan from parts
    auto parts_plan = readFromParts(
        data.getDataPartsVector(), column_names_to_return, metadata_snapshot,
        query_info, context, max_block_size, num_streams,
        max_block_numbers_to_read);
    // if (parts_plan->isInitialized())
    // {
    //     auto adding_missed = std::make_unique<AddingMissedStep>(
    //             parts_plan->getCurrentDataStream(),
    //             metadata_snapshot->getSampleBlock(),
    //             metadata_snapshot->getColumns(), context);

    //     adding_missed->setStepDescription("Add columns missing in destination table");
    //     parts_plan->addStep(std::move(adding_missed));
        
    //     auto actions_dag = ActionsDAG::makeConvertingActions(
    //             parts_plan->getCurrentDataStream().header.getColumnsWithTypeAndName(),
    //             metadata_snapshot->getSampleBlock().getColumnsWithTypeAndName(),
    //             ActionsDAG::MatchColumnsMode::Name);

    //     auto converting = std::make_unique<ExpressionStep>(parts_plan->getCurrentDataStream(), actions_dag);

    //     converting->setStepDescription("Convert destination table columns to Buffer table structure");
    //     parts_plan->addStep(std::move(converting));
    // }
    
    // query_plan from logs, inspire from StorageBuffer

    QueryPlanPtr logs_plan = std::make_unique<QueryPlan>();
    if(data.getSettings()->enable_log_part)
    {
        Pipe pipe_from_logs;
        Pipes pipes_from_logs;

        std::optional<PartitionPruner> log_partition_pruner;
        log_partition_pruner.emplace(metadata_snapshot->getPartitionKey(), query_info, context, false /* strict */);

        for(auto it = data.partition_to_log_parts.begin(); it != data.partition_to_log_parts.end(); ++it)
        {
            auto log_parts = it->second;
            for(auto log_part : log_parts)
            {
                if (log_partition_pruner)
                {
                    if (log_partition_pruner->canBePrunedLogPart(log_part))
                        continue;
                }
                pipes_from_logs.emplace_back(std::make_shared<LogPartSource>(column_names_to_return, *log_part, metadata_snapshot));
            }
        }
        pipe_from_logs = Pipe::unitePipes(std::move(pipes_from_logs));
        if(!pipe_from_logs.empty())
        {
            if (query_info.prewhere_info)
            {
                pipe_from_logs.addSimpleTransform([&](const Block & header)
                {
            ...
            auto read_from_logs = std::make_unique<ReadFromPreparedSource>(std::move(pipe_from_logs));
            read_from_logs->setStepDescription("Read from logs");
            logs_plan->addStep(std::move(read_from_logs));
        }
    }

    // only delta or main have data.
    if (!parts_plan->isInitialized())
    {
        return logs_plan;
    }
    if (!logs_plan->isInitialized())
    {
        return parts_plan;
    }

    /// merge main data and delta data.
    /// TODO: check query plan build detaily.
    DataStreams input_streams;
    auto result_header = parts_plan->getCurrentDataStream().header;
    /// Convert structure from table to structure from buffer.
    if (!blocksHaveEqualStructure(logs_plan->getCurrentDataStream().header, result_header))
    {
        auto convert_actions_dag = ActionsDAG::makeConvertingActions(
                logs_plan->getCurrentDataStream().header.getColumnsWithTypeAndName(),
                result_header.getColumnsWithTypeAndName(),
                ActionsDAG::MatchColumnsMode::Name);

        auto converting = std::make_unique<ExpressionStep>(logs_plan->getCurrentDataStream(), convert_actions_dag);
        logs_plan->addStep(std::move(converting));
    }
    input_streams.emplace_back(parts_plan->getCurrentDataStream());
    input_streams.emplace_back(logs_plan->getCurrentDataStream());

    std::vector<std::unique_ptr<QueryPlan>> plans;
    plans.emplace_back(std::move(parts_plan));
    plans.emplace_back(std::move(logs_plan));
    auto query_plan = std::make_unique<QueryPlan>();
    
    auto union_step = std::make_unique<UnionStep>(std::move(input_streams), result_header);
    union_step->setStepDescription("Unite sources");
    ...
    /// Let's find what range to read from each part.
    {
        ...
        auto process_part = [&](size_t part_index)
        {
            ...
            if (!ranges.ranges.empty())
            {
                ...
                int access_cnt = 0;
                for (auto it = ranges.ranges.begin(); it != ranges.ranges.end(); it++) {
                    access_cnt += it->end - it->begin;
                }
                if (access_cnt > 0) {
                    ...
                    access_cnt = access_cnt * 8192 / parts.size();
                    for (auto name : column_names_to_return) {
                        part->updateColumnSize(name, access_cnt);
                    }
                    for (auto col : part->getColumns()) {
                        if (names.count(col.name) == 0) {
                            part->updateColumnSize(col.name, 0);
                        }
                    }
                }
        ...
        // for calculate part access status.
        // query_info.prewhere_info->part_num = parts.size();
