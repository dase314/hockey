...
struct off_and_lens
{
 ...
 struct Meta {
    ...
    pmem::obj::p<int> cnt;
    pmem::obj::p<int> index_cnt;
 };


 struct pstrhash {
        std::size_t operator()(const pmem::obj::string& key) const {
            return std::hash<std::string>()(key.c_str());
        }
    };
using hashmap_type =
    pmem::obj::concurrent_hash_map<pmem::obj::string, persistent_ptr<Meta>, pstrhash>;
...
public:
    ...
    std::unordered_map<String, MergeTreeLogPartPtrs> partition_to_log_parts;
    ...
    mutable pmem::obj::pool<Tree> pop;

    struct root {
        pmem::obj::persistent_ptr<hashmap_type> pptr;
    };

    mutable pmem::obj::pool<root> pop_meta;

    persistent_ptr<Meta> GetMetaFromDataPart(const std::string& part_name) const;

    bool InsertMetaData(const std::string& part_name, persistent_ptr<Meta> meta_data) const;

    bool DeleteMetaData(const std::string& part_name) const;

    /// create if there isn't an active part.
    MergeTreeLogPartPtr getOrCreateActiveLogPart(MergeTreePartition & partition, const StorageMetadataPtr & metadata_snapshot);

    /// drop log part because it has been flush to persistent storage.
    void dropLogPart(MergeTreePartition & partition, const StorageMetadataPtr & metadata_snapshot);

    /// use when server closing.
    // void flush_all_log_part();
protected:
