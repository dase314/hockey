...
MergeTreeMarksLoader::MergeTreeMarksLoader(
    ...
    MergeTreeDataPartType part_type_)
    : disk(std::move(disk_))
...
MarkCache::MappedPtr MergeTreeMarksLoader::loadMarksImpl()
{
    ...
    if (!index_granularity_info.is_adaptive)
    {
        ...
        if(part_type==MergeTreeDataPartType::IN_NVM)
        {
            buffer = std::make_unique<ReadBufferFromNVM>(fullPath(disk, mrk_path),file_size);
        }
        else
        {
            buffer = disk->readFile(mrk_path, file_size);
        }
