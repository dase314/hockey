#include "MergeTreeDataPartInNVM.h"
#include <Storages/MergeTree/IMergeTreeDataPartWriter.h>
#include <Storages/MergeTree/MergeTreeDataPartWriterInNVM.h>
#include <Storages/MergeTree/MergeTreeReaderInNVM.h>
#include <Poco/File.h>
...
MergeTreeDataPartInNVM::MergeTreeDataPartInNVM(
    MergeTreeData & storage_, const String & name_, const VolumePtr & volume_, const std::optional<String> & relative_path_)
    : IMergeTreeDataPart(storage_, name_, volume_, relative_path_, Type::IN_NVM)
{
    //without decode
...
MergeTreeDataPartInNVM::MergeTreeDataPartInNVM(
    ...
    : IMergeTreeDataPart(storage_, name_, info_, volume_, relative_path_, Type::IN_NVM)
{
    default_codec = CompressionCodecFactory::instance().get("NONE", {});
}

IMergeTreeDataPart::MergeTreeReaderPtr MergeTreeDataPartInNVM::getReader(
...
{
    auto ptr = std::static_pointer_cast<const MergeTreeDataPartInNVM>(shared_from_this());
    return std::make_unique<MergeTreeReaderInNVM>(
        ptr,
        columns_to_read,
        metadata_snapshot,
        uncompressed_cache,
...
IMergeTreeDataPart::MergeTreeWriterPtr MergeTreeDataPartInNVM::getWriter(
...
{
    return std::make_unique<MergeTreeDataPartWriterInNVM>(
        shared_from_this(),
        columns_list,
        metadata_snapshot,
        indices_to_recalc,
...
ColumnSize MergeTreeDataPartInNVM::getColumnSizeImpl(
...
void MergeTreeDataPartInNVM::loadIndexGranularity()
{
    ...
    if (!volume->getDisk()->exists(marks_file_path))
        throw Exception(
            "Marks file '" + fullPath(volume->getDisk(), marks_file_path) + "' doesn't exist", ErrorCodes::NO_FILE_IN_DATA_PART);
    ...
    else
    {
        ...
        if (index_granularity.getMarksCount() * index_granularity_info.getMarkSizeInBytes() != marks_file_size)
            throw Exception(
                "Cannot read all marks from file " + fullPath(volume->getDisk(), marks_file_path), ErrorCodes::CANNOT_READ_ALL_DATA);
    }
...
void MergeTreeDataPartInNVM::checkConsistency(bool require_part_metadata) const
{
    ...
    if (!checksums.empty())
    {
        if (require_part_metadata)
        {
            for (const NameAndTypePair & name_type : columns)
            {
                ...
                name_type.type->enumerateStreams(
                    [&](const IDataType::SubstreamPath & substream_path, const IDataType & /* substream_type */) {
                        ...
                        if (!checksums.files.count(mrk_file_name))
                            throw Exception(
                                "No " + mrk_file_name + " file checksum for column " + name_type.name + " in part "
                                    + fullPath(volume->getDisk(), path),
                                ErrorCodes::NO_FILE_IN_DATA_PART);
                        if (!checksums.files.count(bin_file_name))
                            throw Exception(
                                "No " + bin_file_name + " file checksum for column " + name_type.name + " in part "
                                    + fullPath(volume->getDisk(), path),
                                ErrorCodes::NO_FILE_IN_DATA_PART);
                    },
                    stream_path);
    ...
    else
    {
        ...
        for (const NameAndTypePair & name_type : columns)
        {
            name_type.type->enumerateStreams([&](const IDataType::SubstreamPath & substream_path, const IDataType & /* substream_type */) {
                auto file_path
                    = path + IDataType::getFileNameForStream(name_type.name, substream_path) + index_granularity_info.marks_file_extension;
                ...
                if (volume->getDisk()->exists(file_path))
                {
                    ...
                    if (!file_size)
                        throw Exception(
                            "Part " + path + " is broken: " + fullPath(volume->getDisk(), file_path) + " is empty.",
                    ...
                    if (!marks_size)
                        ...
                        throw Exception(
                            "Part " + path + " is broken: marks have different sizes.", ErrorCodes::BAD_SIZE_OF_FILE_IN_DATA_PART);
                }
            });
        }
    }
...
bool MergeTreeDataPartInNVM::hasColumnFiles(const String & column_name, const IDataType & type) const
{
    bool res = true;
...
String MergeTreeDataPartInNVM::getFileNameForColumn(const NameAndTypePair & column) const
{
    String filename;
...
void MergeTreeDataPartInNVM::calculateEachColumnSizes(ColumnSizeByName & each_columns_size, ColumnSize & total_size) const
{
