...
MergeTreeReaderInNVM::MergeTreeReaderInNVM(
    DataPartInNVMPtr data_part_,
...
size_t MergeTreeReaderInNVM::readRows(size_t from_mark, bool continue_reading, size_t max_rows_to_read, Columns & res_columns)
{
    size_t read_rows = 0;
    try
    {
        ...
        for (size_t pos = 0; pos < num_columns; ++pos, ++name_and_type)
        {
            ...
            if (const auto * type_arr = typeid_cast<const DataTypeArray *>(type.get()))
            {
                ...
                if (!append)
                    column = ColumnArray::create(type_arr->getNestedType()->createColumn(), it_inserted.first->second)->assumeMutable();
            }
    ...
    catch (Exception & e)
    {
        ...
        /// Better diagnostics.
        e.addMessage(
            "(while reading from part " + data_part->getFullPath()
            + " "
              "from mark "
            + toString(from_mark)
            + " "
              "with max_rows_to_read = "
            + toString(max_rows_to_read) + ")");
        throw;
    }
    catch (...)
    {
...
void MergeTreeReaderInNVM::addStreams(
    const String & name, const IDataType & type, const ReadBufferFromFileBase::ProfileCallback & profile_callback, clockid_t clock_type)
{
...
void MergeTreeReaderInNVM::readData(
    const String & name,
