...
IMergeTreeDataPart::Checksums checkDataPartInPMCS(const DataPartPMCSPtr & data_part)
{
    IMergeTreeDataPart::Checksums data_checksums;
    data_checksums.files["data.bin"] = data_part->calculateBlockChecksum();
    // data_part->checksums.checkEqual(data_checksums, true);
    ...
    std::function<bool()> is_cancelled)
{
    ...
    if (auto part_in_pmcs = asPMCSPart(data_part))
        return checkDataPartInPMCS(part_in_pmcs);
