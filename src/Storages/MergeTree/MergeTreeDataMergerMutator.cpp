    ...
    const Names & deduplicate_by_columns)
{
    ...
    if (data.getSettings()->enable_pmcs_part && data.getSettings()->enable_hybrid) {
        ...
        for (auto part : future_part.parts) {
            future_worth += part->getWorth();
        }
        future_worth = future_worth / future_part.parts.size();
        auto disks = data.getStoragePolicy()->getDisks();
        if (future_worth <= worth_threadhold) {
            future_part.type = MergeTreeDataPartType::WIDE;
            for (auto d : disks) {
                if (d->getDiskType() == "SSD") {
                    disk = d;
                    break;
                }
            }
    ...
    // here to update new_part worth, to determine where to put the new_part.
