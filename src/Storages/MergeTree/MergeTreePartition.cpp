...
void MergeTreePartition::loadPmem(const MergeTreeData & storage, String data)
{
    auto metadata_snapshot = storage.getInMemoryMetadataPtr();
    if (!metadata_snapshot->hasPartitionKey())
        return;
    ...
    value.resize(partition_key_sample.columns());
    for (size_t i = 0; i < partition_key_sample.columns(); ++i)
        partition_key_sample.getByPosition(i).type->deserializeBinary(value[i], in);
}
...
std::string MergeTreePartition::toStringData(const MergeTreeData & storage) const
{
    auto metadata_snapshot = storage.getInMemoryMetadataPtr();
    const auto & partition_key_sample = metadata_snapshot->getPartitionKey().sample_block;
    WriteBufferFromOwnString out;
    for (size_t i = 0; i < value.size(); ++i)
        partition_key_sample.getByPosition(i).type->serializeBinary(value[i], out);
    return out.str();
}
