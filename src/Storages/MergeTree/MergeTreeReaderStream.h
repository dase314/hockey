...
public:
    MergeTreeReaderStream(
        ...
        const ReadBufferFromFileBase::ProfileCallback & profile_callback, clockid_t clock_type, MergeTreeDataPartType type=MergeTreeDataPartType::WIDE);

    void seekToMark(size_t index);

    void seekToStart();
    std::unique_ptr<ReadBufferFromFileBase> openNVMFile(const String & path)
{
    return std::make_unique<ReadBufferFromNVM>(path);
}
