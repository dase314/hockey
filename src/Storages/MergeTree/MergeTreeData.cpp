...
#include <Storages/MergeTree/MergeTreeDataPartInNVM.h>
    ...
    , parts_mover(this)
{
    ...
    // create log pool in pmem
    if (storage_settings.get()->enable_log_part)
    {   
        for (const auto & [path, disk] : getRelativeDataPathsWithDisks())
        {
            if (disk->getName() == "default" || disk->getDiskType() == "PMEM") {
                auto log_path = path + "log.pool";
                if (disk->exists(log_path))
                {
                    pop = pool<Tree>::open(disk->getPath() + log_path,"");
                } else {
                    pop = pool<Tree>::create(disk->getPath() + log_path,"", LOG_POOL_SIZE, S_IWUSR | S_IRUSR);
                }
                break;
            }
        }
    ...
    // create global meta pool
    if (storage_settings.get()->enable_pmcs_part) {
        for (const auto & [path, disk] : getRelativeDataPathsWithDisks())
        {
            if (disk->getName() == "default" || disk->getDiskType() == "PMEM") {
                auto meta_path = path + "meta.pool";
                if (disk->exists(meta_path))
                {
                    pop_meta = pool<root>::open(disk->getPath() + meta_path, "");
                } else {
                    pop_meta = pool<root>::create(disk->getPath() + meta_path, "", META_POOL_SIZE, S_IWUSR | S_IRUSR);
                }
                break;
            }
        }
...
persistent_ptr<Meta> MergeTreeData::GetMetaFromDataPart(const std::string& part_name) const {
    ...
    bool res = map->find(acc, *p_name);
    persistent_ptr<Meta> meta_data;
    if (res) {
        meta_data = acc->second;
    }
    acc.release();
    pmem::obj::transaction::run(pop_meta, [&] {
        pmem::obj::delete_persistent<pmem::obj::string>(p_name);
    });
    return meta_data;
}

bool MergeTreeData::InsertMetaData(const std::string& part_name, persistent_ptr<Meta> meta_data) const {
    persistent_ptr<pmem::obj::string> p_name;
    auto &map = pop_meta.root()->pptr;
    pmem::obj::transaction::run(pop_meta, [&] {
            p_name = pmem::obj::make_persistent<pmem::obj::string>(part_name);
    });
    map->insert_or_assign(*p_name, meta_data);
    pmem::obj::transaction::run(pop_meta, [&] {
        pmem::obj::delete_persistent<pmem::obj::string>(p_name);
    });
    return true;
...
double MergeTreeData::getMergeThreshold() const {
    std::vector<double> worths;
    auto parts = getAllDataPartsVector();
    if (parts.empty()) {
        return 0;
    }
    for (auto part : parts) {
        worths.push_back(part->getWorth());
    }
    std::sort(worths.begin(), worths.end());
    return worths[parts.size() / 2];
}
...
MergeTreeDataPartType MergeTreeData::choosePartType(size_t bytes_uncompressed, size_t rows_count) const
{
    ...
    const auto settings=getSettings();
    if(settings->enable_pmcs_part)
    {
        return MergeTreeDataPartType::PMCS;
    }
    if(settings->enable_nvm_part)
    {
        return MergeTreeDataPartType::IN_NVM;
    }
...
{//merge时需要选择part type时会调用这个part choose 算法.
    //auto settings = global_context.getSettingsRef();
    const auto settings=getSettings();
    if(settings->enable_pmcs_part)
    {
    ...
    const VolumePtr & volume, const String & relative_path) const
{
    if (type == MergeTreeDataPartType::COMPACT)
        ...
        return std::make_shared<MergeTreeDataPartInNVM>(*this, name, part_info, volume, relative_path);
    else if (type==MergeTreeDataPartType::PMCS)
        return std::make_shared<MergeTreeDataPartPMCS>(*this, name, part_info, volume, relative_path);
    else
    ...
    const VolumePtr & volume, const String & relative_path) const
{
    ...
    if (mrk_ext)
    {
        if(getSettings()->enable_nvm_part)
        {
            type = MergeTreeDataPartType::IN_NVM;
        }else{
            type = getPartTypeFromMarkExtension(*mrk_ext);
        }
    }else if(volume->getDisk()->exists(full_path+"meta")){
        type = MergeTreeDataPartType::PMCS;
    }
    else
    {
...
MergeTreeLogPartPtr MergeTreeData::getOrCreateActiveLogPart(MergeTreePartition & partition, const StorageMetadataPtr & metadata_snapshot)
{
    auto partition_id = partition.getID(metadata_snapshot->getPartitionKey().sample_block);
    
    if(partition_to_log_parts[partition_id].size() == 0) {
        partition_to_log_parts[partition_id].emplace_back(std::make_shared<MergeTreeLogPart>(*this, partition, partition_id));
    }
    
    auto log_part = partition_to_log_parts[partition_id].back();
    if(log_part->state != MergeTreeLogPart::LogState::Active)
    {
        partition_to_log_parts[partition_id].emplace_back(std::make_shared<MergeTreeLogPart>(*this, partition, partition_id));
        return partition_to_log_parts[partition_id].back();
    }
    
    return log_part;
}

void MergeTreeData::dropLogPart(MergeTreePartition & partition, const StorageMetadataPtr & metadata_snapshot)
{
    auto partition_id = partition.getID(metadata_snapshot->getPartitionKey().sample_block);

    bool dropped = false;
    for(auto log_part = partition_to_log_parts[partition_id].begin(); log_part != partition_to_log_parts[partition_id].end(); log_part++)
    {
        if((*log_part)->state == MergeTreeLogPart::LogState::Completed)
        {
            (*log_part)->shutdown();
            partition_to_log_parts[partition_id].erase(log_part);
            dropped = true;
            return ;
        }
    }
    if(!dropped)
    {
        throw Exception("no drop log part", ErrorCodes::LOGICAL_ERROR);
    }

}

// void MergeTreeData::flush_all_log_part()
// {
//     for(auto & it : partition_to_log_parts)
//     {
//          for(auto & log_part : it.second)
//          {
//             Stopwatch watch;
//             for(auto & col : log_part->data)
//             {
//                 auto mut_col = col.column->assumeMutable();
...
//             MergeTreeData::MutableDataPartPtr part = writer.writeTempPart(tmp_block, metadata_snapshot, false);
//             renameTempPartAndAdd(part, &increment);
//             log_part->shutdown();

//             PartLog::addNewPart(global_context, part, watch.elapsed());
//          }
//     }
// }
}
