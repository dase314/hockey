...
void IMergeTreeDataPart::MinMaxIndex::loadPmem(const MergeTreeData & data, std::vector<std::pair<String,String>> raw_data)
{
    size_t minmax_idx_size = data.minmax_idx_column_types.size();
    hyperrectangle.reserve(minmax_idx_size);
    for (size_t i = 0; i < minmax_idx_size; ++i)
    {
        ...
        // doesn't check raw_data.first(file_name), usually there is one minmax column 
        ReadBufferFromString in(raw_data[i].second);
        Field min_val;
        data_type->deserializeBinary(min_val, in);
        Field max_val;
        data_type->deserializeBinary(max_val, in);
...
std::vector<std::pair<std::string,std::string>> IMergeTreeDataPart::MinMaxIndex::toString(const MergeTreeData & data) const
{
    auto & column_names = data.minmax_idx_columns;
    auto & data_types = data.minmax_idx_column_types;
    ...
    for (size_t i = 0; i < column_names.size(); ++i)
    {
        ...
        data_type->serializeBinary(hyperrectangle[i].left, out);
        data_type->serializeBinary(hyperrectangle[i].right, out);
        res.push_back(std::make_pair(file_name,out.str()));
    }
    return res;
}
...
{
    if (!isStoredOnDisk() && getType() != MergeTreeDataPartType::PMCS)
...
void IMergeTreeDataPart::remove() const
{
    ...
    if(getType() == MergeTreeDataPartType::PMCS)
    {
        volume->getDisk()->removeRecursive(from + "/");
        storage.DeleteMetaData(name);
        return ;
    }
    if (volume->getDisk()->exists(to))
    {
...
double IMergeTreeDataPart::getWorth() const {
    double worth = 0;
    for(auto col : columns) {
        auto col_size = getColumnSize(col.name, *col.type);
        worth += col_size.access_cnt * col.type->getSizeOfValueInMemory();
    }
    if (rows_count > 0)
        worth /= rows_count;
    else return 0;
