...
MergeTreeReaderStream::MergeTreeReaderStream(
        ...
        const ReadBufferFromFileBase::ProfileCallback & profile_callback, clockid_t clock_type, MergeTreeDataPartType type)
        ...
        , marks_loader(disk, mark_cache, index_granularity_info->getMarksFilePath(path_prefix),
            marks_count, *index_granularity_info, save_marks_in_cache,1,type)
{
    ...
    if (uncompressed_cache)
    {
        ...
        }else if(type==MergeTreeDataPartType::IN_NVM)
        {
            ...
            [this, buffer_size, sum_mark_range_bytes, &settings]()
            {
                return openNVMFile(fullPath(disk, path_prefix + data_file_extension));
            },
            uncompressed_cache);
            if (profile_callback)
            buffer->setProfileCallback(profile_callback, clock_type);
    ...
    {   
        std::unique_ptr<CompressedReadBufferFromFile> buffer;
        if(type==MergeTreeDataPartType::WIDE)
        {
            buffer = std::make_unique<CompressedReadBufferFromFile>(
        ...
        {
            buffer = std::make_unique<CompressedReadBufferFromFile>(openNVMFile(fullPath(disk, path_prefix + data_file_extension)));
        }
