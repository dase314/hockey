...
void MergeTreeDataPartWriterOnDisk::initPrimaryIndex()
{
    ...
    {
        if(data_part->getType()==MergeTreeDataPartType::IN_NVM)
        {
            //index_file_stream = data_part->volume->getDisk()->writeFile(part_path + "primary.idx", DBMS_DEFAULT_BUFFER_SIZE, WriteMode::Rewrite);
            index_file_stream=std::make_unique<WriteBufferFromNVM>(fullPath(data_part->volume->getDisk(), part_path + "primary.idx"),DBMS_DEFAULT_BUFFER_SIZE);
