...
void StorageMergeTree::shutdown()
{
    ...
    //     {
    //         // TODO: flush to persistent storage
    //         // i->shutdown();
    ...
    //         // LOG_INFO(log, "log_part.use_count"+std::to_string(i.use_count()));
    //     }
    // }
    for(auto & it : partition_to_log_parts)
    {
    ...
    if(getSettings()->enable_log_part)
    {
        pop.close();
    }
    if(getSettings()->enable_pmcs_part)
    {
        pop_meta.close();
    }
    
    LOG_INFO(log, "Call MergeTree shutdown");
}
