...
public:
    ...
    DiskLocal(const String & name_, const String & path_, UInt64 keep_free_space_bytes_, const String & disk_type_ = "SSD")
        : name(name_), disk_path(path_), keep_free_space_bytes(keep_free_space_bytes_), disk_type(disk_type_)
    {
        if (disk_path.back() != '/')
    ...
    const String getDiskType() const override {return disk_type;}
