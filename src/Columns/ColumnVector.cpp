...
template <typename T>
MutableColumnPtr ColumnVector<T>::cloneEmptyFromPmem(pmem::obj::persistent_ptr<pmem::obj::string> & str) const
{
    auto res = this->create();
    auto & new_col = static_cast<Self &>(*res);
    new_col.p_data = str;
...
template <typename T>
void ColumnVector<T>::insertPmemRangeFrom(const IColumn & src, size_t start, size_t length)
{
    const ColumnVector & src_vec = assert_cast<const ColumnVector &>(src);
    ...
    if (start + length > src_vec.data.size())
        throw Exception("Parameters start = "
            + toString(start) + ", length = "
            + toString(length) + " are out of bound in ColumnVector<T>::insertPmemRangeFrom method"
            " (data.size() = " + toString(src_vec.data.size()) + ").",
            ErrorCodes::PARAMETER_OUT_OF_BOUND);

    size_t old_size = (*p_data).size();
    (*p_data).resize(old_size + length * sizeof(src_vec.data[0]));
    memcpy((*p_data).data() + old_size, &src_vec.data[start], length * sizeof(data[0]));
    
}

template <typename T>
void ColumnVector<T>::insertRangeFromPmem(const IColumn & src, size_t start, size_t length)
{
    const ColumnVector & src_vec = assert_cast<const ColumnVector &>(src);

    if (start + length > src_vec.p_data->size()/sizeof(T))
        throw Exception("Parameters start = "
            + toString(start) + ", length = "
            + toString(length) + " are out of bound in ColumnVector<T>::insertRangeFromPmem method"
            " (data.size() = " + toString(src_vec.p_data->size()/sizeof(T)) + ").",
            ErrorCodes::PARAMETER_OUT_OF_BOUND);

    size_t old_size = data.size();
    data.resize(old_size + length);
    memcpy(data.data() + old_size, &(*src_vec.p_data)[start], length * sizeof(T));
}

template <typename T>
void ColumnVector<T>::insertMemRangeFrom(size_t, size_t length)
{
    data.resize(length);

    memcpy(data.data(), (*p_data).data(), length * sizeof(data[0]));
