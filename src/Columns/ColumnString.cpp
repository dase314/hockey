...
MutableColumnPtr ColumnString::cloneEmptyFromPmem(pmem::obj::persistent_ptr<pmem::obj::string> & str) const
{
    auto res = ColumnString::create();
    res->p_data = str;
...
void ColumnString::insertRangeFrom(const IColumn & src, size_t start, size_t length)
{
    ...
    if(start != 0)
    {
        nested_offset = src_concrete.offsetAt(start);
    }else{
        nested_offset = 0;
    }
...
void ColumnString::insertPmemRangeFrom(const IColumn & src, size_t start, size_t length)
{
    if (length == 0)
        return;
    ...
    size_t old_chars_size = (*p_data).size();
    (*p_data).resize(old_chars_size + nested_length);
    memcpy((*p_data).data() + old_chars_size, &src_concrete.chars[nested_offset], nested_length);
...
void ColumnString::insertRangeFromPmem(const IColumn & src, size_t start, size_t length)
{
    if (length == 0)
        return;
    ...
    if (start + length > src_concrete.offsets.size())
        throw Exception("Parameter out of bound in IColumnString::insertRangeFromPmem method.",
            ErrorCodes::PARAMETER_OUT_OF_BOUND);
    size_t nested_offset;
    if(start != 0)
    {
    ...
    size_t old_chars_size = chars.size();
    chars.resize(old_chars_size + nested_length);
    memcpy(&chars[old_chars_size], &(*src_concrete.p_data)[nested_offset], nested_length);
...
void ColumnString::insertMemRangeFrom(size_t, size_t length)
{
    // size_t old_size = chars.size();
    size_t nested_length = offsets[length - 1];
    chars.resize(nested_length);

    memcpy(chars.data(), (*p_data).data(), nested_length * sizeof(chars[0]));
