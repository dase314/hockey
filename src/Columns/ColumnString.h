...
private:
    ...
    pmem::obj::persistent_ptr<pmem::obj::string> p_data;
...
public:
    ...
    void setOffsetsAddr(char * addr, size_t size)
    {
        offsets.setAddr(addr, size);
    }

    void setDataAddr(char * addr, size_t size)
    {
        chars.setAddr(addr, size);
    }
    ...
    size_t byteSizePmem() const override
    {
        return chars.size() + offsets.size() * sizeof(offsets[0]) + sizeof(UInt64);
    }
    ...
    MutableColumnPtr cloneEmptyFromPmem(pmem::obj::persistent_ptr<pmem::obj::string> & str) const override;

    // MutableColumnPtr cloneEmptyFromPmem(pmem::obj::persistent_ptr<pmem::obj::string> & str_off, pmem::obj::persistent_ptr<pmem::obj::string> & str_data) const override;
...
#pragma GCC diagnostic pop
    ...
    void insertFrom(const IColumn & src_, size_t n) override
    {
        ...
        if(n)
        {
            size_to_append = src.offsets[n] - src.offsets[n - 1];  /// -1th index is Ok, see PaddedPODArray.
        }else if(n == 0){
    ...
    void insertPmemRangeFrom(const IColumn & src, size_t start, size_t length) override;

    void insertRangeFromPmem(const IColumn & src, size_t start, size_t length) override;

    void insertMemRangeFrom(size_t start, size_t length) override;
