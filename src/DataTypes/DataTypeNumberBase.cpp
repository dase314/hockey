...
template <typename T>
void DataTypeNumberBase<T>::serializeToPmem(const IColumn & column, char * pmem_addr) const
{
    const typename ColumnVector<T>::Container & x = typeid_cast<const ColumnVector<T> &>(column).getData();

    size_t size = x.size();
    pmem_memcpy_nodrain(pmem_addr, reinterpret_cast<const char *>(&x[0]), sizeof(typename ColumnVector<T>::ValueType) * size);
}

template <typename T>
void DataTypeNumberBase<T>::deserializeFromPmem(IColumn & column, char * pmem_addr, size_t offset) const
{
    //typename ColumnVector<T>::Container & x = typeid_cast<ColumnVector<T> &>(column).getData();
    //size_t initial_size = x.size();
    //x.resize(initial_size + offset / sizeof(typename ColumnVector<T>::ValueType));
    typeid_cast<ColumnVector<T> &>(column).setAddr(pmem_addr,offset);
}

template <typename T>
void DataTypeNumberBase<T>::deserializeFromPmemCpy(IColumn & column, char * pmem_addr, size_t offset) const
{
    typename ColumnVector<T>::Container & x = typeid_cast<ColumnVector<T> &>(column).getData();
    size_t initial_size = x.size();
    x.resize(initial_size + offset);
    ::memcpy(reinterpret_cast<char*>(&x[initial_size]), pmem_addr, offset);
    x.resize(initial_size + offset / sizeof(typename ColumnVector<T>::ValueType));
}
