...
void DataTypeString::serializeToPmem(const IColumn & column, char * pmem_addr) const
{
    const ColumnString & column_string = typeid_cast<const ColumnString &>(column);
    const ColumnString::Chars & data = column_string.getChars();
    const ColumnString::Offsets & offsets = column_string.getOffsets();

    UInt64 off_size = offsets.size() * sizeof(offsets[0]);
    pmem_memcpy_nodrain(pmem_addr,reinterpret_cast<const char *>(&off_size),sizeof(UInt64));
    pmem_memcpy_nodrain(pmem_addr+sizeof(UInt64), reinterpret_cast<const char *>(&offsets[0]), off_size);
    pmem_memcpy_nodrain(pmem_addr+sizeof(UInt64) + off_size, reinterpret_cast<const char *>(&data[0]), data.size());
}

void DataTypeString::deserializeFromPmem(IColumn & column, char * pmem_addr, size_t offset) const
{
    UInt64 off_size;
    memcpy(&off_size,pmem_addr,sizeof(UInt64));
    typeid_cast<ColumnString &>(column).setOffsetsAddr(pmem_addr+sizeof(UInt64),off_size);
    
    typeid_cast<ColumnString &>(column).setDataAddr(pmem_addr+sizeof(UInt64)+off_size,offset -  off_size - sizeof(UInt64));
}
