...
public:
    ...
    /// serialize column to pmem, just for test, maybe doen't support all type
    virtual void serializeToPmem(const IColumn & column, char * pmem_addr) const;

    /// zero memory copy
    virtual void deserializeFromPmem(IColumn & column, char * pmem_addr, size_t offset) const;

    /// with memory copy for index
    virtual void deserializeFromPmemCpy(IColumn & column, char * pmem_addr, size_t offset) const;
